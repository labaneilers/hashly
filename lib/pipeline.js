...
var hashpattern = require("./hashpattern");
var fsutil = require("./filesystemutil");
...
exports.processDirectory = function (sourceDir, targetDir, options) {
    ...
    var serializer = getSerializer(_options.manifestFormat);

    var manifestPath = path.join(targetDir, serializer.fileName);
...
var getSerializer = function(manifestFormat) {
    manifestFormat = (manifestFormat || "").toLowerCase();

    if (!_manifestFormats[manifestFormat]) {
        manifestFormat = "json";
    }

    console.log("using serializer: " + manifestFormat);
...
exports.clean = function (directory, options) {
    ...
    var serializer = getSerializer(options.manifestFormat);

    deleteFileSync(path.join(directory, serializer.fileName));
